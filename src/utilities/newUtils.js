...
console.log(getLinesOfN(testBoard31, 2, false))
...
function getLinesOfN(board, n, player){
    const height = board[0].length
    const width = board.length
    const emptySlot = null
    ...
    const isExactlyLineOfN = (line, firstSlotRow, firstSlotCol) => {
    ...
    for(let row = 0; row < width; row++){
        for(let col = 0; col < height; col++){
            ...
            if(slot === emptySlot || slot !== player){
                continue //don't check starting from empty slots or slots that don't match the player
            }

            if(col + (n - 1) < height){
                const lineToCheck = [slot, //look right
                                    board[row][col + 1], 
                                    col + 2 <= 5 ? board[row][col + 2] : null, 
                                    col + 3 <= 5 ? board[row][col + 3] : null]
            ...
            if(row + (n - 1) < width){
                ...
                if(col + (n - 1) < height){
                    const lineToCheck = [slot, //look up & right
                                        board[row + 1][col + 1],
                                        (col + 2 <= 5) && (row + 2 <= 6) ? board[row + 2][col + 2] : null,
                                        (col + 3 <= 5) && (row + 3 <= 6) ? board[row + 3][col + 3] : null]
                ...
                if(col - 3 >= 0){
                    const lineToCheck = [slot, //look up & left
                                        ...
                                        (col - 3 >= 0) && (row + 3 <= 6) ? board[row + 3][col - 3] : null]
